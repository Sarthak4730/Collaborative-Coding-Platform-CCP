    ...
    return <>
        ...
        */}
        <form onSubmit={handleSubmit} className="bg-gray-200 w-5/6 h-96 md:w-[50vw] md:h-[75vh] mx-auto mt-64 md:mt-[10vh] rounded-2xl flex flex-col items-center justify-evenly">
            <h1 className="text-center text-2xl md:text-3xl font-bold">Register New User</h1>

            <input className="border-2 border-blue-500 bg-white rounded-2xl pl-4 w-2/3 py-1 md:h-10 md:w-[20vw]" type="text" name="username" placeholder="Enter Username" onChange={handleChange} required/>
            ...
            <div className="login-div w-5/6 flex justify-center gap-3 items-center">
                <p className="text-xs md:text-lg font-semibold">Already have an account?</p>
                <button onClick={ () => { navigate("/login") } } type="submit" className="w-1/3 md:w-[12.5vw] md:h-[7.5vh] rounded-xl cursor-pointer py-1 text-lg hover:scale-105 font-bold border-3 border-blue-500 ">Login</button>
            </div>
        </form>
    </>
};